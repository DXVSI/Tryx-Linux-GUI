...
- Turris Apply and delete no longer wait three seconds for an acknowledgement
  the device never sends; both are still confirmed by reading the state back.
- In the Flatpak, a file dragged from a host file manager is resolved through
  the FileTransfer portal, so drag and drop works without home directory
  access.
